...
Use this skill in the following scenarios:
    ...
    Design data warehouse from scratch follow professional design principles and best practices.
    Layered data modeling design leverage the medallion architecture, the Kimball architecture and Alibaba's OneData concept.
    New Source Integration: When ingesting data from a new source system into the ODS layer.
    Data model Improvemet: When exsiting model requires to split and move sql logic into different layers to improve the felxibility, maintainability, and performance.
    Performance Optimization: When existing queries are slow due to poor schema design, requiring denormalization or aggregation strategies.
    Schema Evolution: When source systems change structure, requiring updates to DWD/DIM tables and SCD handling.
    New Analytics Requirements: When business stakeholders request new metrics, reports, or dashboards requiring new DWS or ADS tables.
    Data Mart Creation: When building subject-area specific data marts for specific departments (Finance, Marketing, Operations).
    Refactoring: When cleaning up technical debt, inconsistent metrics, or redundant tables in the warehouse.
